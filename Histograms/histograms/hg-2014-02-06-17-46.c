...
int main(int argc, char**argv) {
    ...
    L = atoi(argv[1]);
    if (L > MAXL) {
        fprintf(stderr, "L must be less than %d. You have entered %d.\n", MAXL, L);
        exit(100);
    }
    if (L < 1) {
        fprintf(stderr, "L must be greater than 0. You have entered %d.\n", L);
        exit(101);
    }
    L2 = L * L;
    if (L2 > MAXL2) {
    ...
    for (ifile = 6; ifile < argc; ifile++)
    {
        ...
        while (!feof(fin)) {
            ...
            if ((0 < t) && (t < L1)) {
#endif
            /* Check for maximal value of each parameter */
            if (t > t_max) t_max = t;
            if (s > s_max) s_max = s;
            if (a > a_max) a_max = a;
            if (x > x_max) x_max = x;
            /* Store data to histogram arrays */
#if _NO_BUMP==0
            if (0 <= t && t < L1) {
#endif
                t_histogram[t]++;
                ...
                fprintf(stderr, "t has invalid value %d in file '%s'.\n", t, argv[ifile]);
                finalize();
                exit(10);
            }
#endif
            if (0 <= x && x < XDIM) {
                ...
                fprintf(stderr, "x has invalid value %d in file '%s'.\n", x, argv[ifile]);
                finalize();
                exit(10);
            }
            if (s < L2) {
