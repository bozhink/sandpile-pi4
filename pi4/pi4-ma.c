...
int alpha;    /* Parameter, which determines the probability of left-right transition */
int mu;       /* Parameter, which determines the number of jumping particles */
...
int main(int argc, char**argv) {
    ...
    if (argc<5) {
        fprintf(stderr, "Usage: %s <N> <L> <alpha> <mu> [<FN>]\n", argv[0]);
        fprintf(stderr, "Where:\n");
        fprintf(stderr, "\t    N =  number of iterations\n");
        fprintf(stderr, "\t    L =  lattice size\n");
        fprintf(stderr, "\talpha = alpha parameter\n");
        fprintf(stderr, "\t   mu = mu parameter\n");
    ...
    L = atoi(argv[2]); fprintf(stderr, "Lattice size = %d\n", L);
    alpha = atoi(argv[3]); fprintf(stderr, "alpha = %d\n", alpha);
    mu = atoi (argv[4]); fprintf(stderr, "mu = %d\n", mu);
    ...
    do {
        ...
        for (j=0; j<i1; j++) {
            while (lattice[i][j] >= THRESHOLD) {
                ...
                switch (rand() % mu) {
                    case 0: /* Tansition of one particle */
                        lattice[i][j] += 1;
                        switch (rand() % alpha) {
                            case 0:
                                lattice[i1][j] += THRESHOLD-1;
                                break;
                            default:
                                lattice[i1][j+1] += THRESHOLD-1;
                        }
                        break;
                    default:
                        switch (rand() % alpha) {
